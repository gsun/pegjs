...
class GenerateBytecodeClass extends AstVisitor {
    ...
    addFunctionConst(params, code) {
		...
		let index = this.funconsts.indexOf(value);

        return index === -1 ? this.funconsts.push(value) - 1 : index;
    }
	
    addExpConst(value) {
        let index = this.expconsts.indexOf(value);

        return index === -1 ? this.expconsts.push(value) - 1 : index;
    }
	
    addRegConst(value) {
        let index = this.regconsts.indexOf(value);

        return index === -1 ? this.regconsts.push(value) - 1 : index;
    }
    ...
    grammar(node) {
		...
		node.funconsts = this.funconsts;
		node.expconsts = this.expconsts;
		node.regconsts = this.regconsts;
    }
    ...
    named(node, context) {
        let nameIndex = this.addExpConst(`peg$otherExpectation("${js.stringEscape(node.name)}")`);
    ...
    literal(node) {
        if (node.value.length > 0) {
            ...
            let expectedIndex = this.addExpConst(`peg$literalExpectation("${js.stringEscape(node.value)}", ${node.ignoreCase})`);
    ...
    class(node) {
        ...
        let regexpIndex = this.addRegConst(regexp);
        let expectedIndex = this.addExpConst(`peg$classExpectation(${parts}, ${node.inverted}, ${node.ignoreCase})`);
    ...
    any() {
        let expectedIndex = this.addExpConst("peg$anyExpectation()");
