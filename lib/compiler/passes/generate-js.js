...
function generateJS(ast, options) {
  ...
  function generateTables() {
    if (options.optimize === "size") {
      let bytecode =  indent2(ast.rules.map(rule =>
             `peg$decode("${js.stringEscape(rule.bytecode.map(b => String.fromCharCode(b + 32)).join(""))}")`
           ).join(",\n"));
      return `
        var peg$consts = [${indent2(ast.consts.join(",\n"))}];

        var peg$bytecode = [${bytecode}];
      `;
    } else {
          ...
          case op.MATCH_STRING:       // MATCH_STRING s, a, f, ...
            compileCondition(
              eval(ast.consts[bc[ip + 1]]).length > 1
                ? `input.substr(peg$currPos, ${eval(ast.consts[bc[ip + 1]]).length}) === ${c(bc[ip + 1])}`
                : `input.charCodeAt(peg$currPos) === ${eval(ast.consts[bc[ip + 1]]).charCodeAt(0)}`,
              1
            );
            break;
