  ...
  function generateTables() {
    if (options.optimize === "size") {
      return `
		...
		var peg$funconsts = [${ast.funconsts.join(",\n")}];
		
		var peg$expconsts = [${ast.expconsts.join(",\n")}];
		
		var peg$regconsts = [${ast.regconsts.join(",\n")}];
      ...
      return `${ast.consts.map((c, i) => `var peg$c${i}  = ${c};`).join("\n")}
	          ${ast.funconsts.map((c, i) => `var peg$cf${i}  = ${c};`).join("\n")}
			  ${ast.expconsts.map((c, i) => `var peg$cp${i}  = ${c};`).join("\n")}
			  ${ast.regconsts.map((c, i) => `var peg$cr${i}  = ${c};`).join("\n")}
	         `;
    }
  }
    ...
    function generateCall() {
      ...
      return `
        ...
        stack.splice(
          ...
          bc[ip + 2],
          peg$funconsts[bc[ip + 1]].apply(null, params)
        );
          ...
          while (ip < end) {
            switch (bc[ip]) {
              ...
              case ${op.MATCH_REGEXP}:       // MATCH_REGEXP r, a, f, ...
                ${generateCondition("peg$regconsts[bc[ip + 1]].test(input.charAt(peg$currPos))", 1)}
              ...
              case ${op.FAIL}:               // FAIL e
                ...
                if (peg$silentFails === 0) {
                  peg$fail(peg$expconsts[bc[ip + 1]]);
                }
                ip += 2;
                break;
  ...
  function generateRuleFunction(rule) {
    ...
    function compile(bc) {
      ...
      while (ip < end) {
        ...
        case op.MATCH_REGEXP: // MATCH_REGEXP r, a, f, ...
          compileCondition(
            `${cr(bc[ip + 1])}.test(input.charAt(peg$currPos)) /* op.MATCH_REGEXP */`, 
            1);
          break;
        ...
        case op.FAIL: // FAIL e
          ...
          parts.push(`if (peg$silentFails === 0) { peg$fail(${cp(bc[ip + 1])}); }`);
          ip += 2;
          break;
